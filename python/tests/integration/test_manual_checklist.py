...
@pytest.mark.asyncio
async def test_403_fatal_failure() -> None:
...
@pytest.mark.asyncio
async def test_500_transient_backoff_and_retry() -> None:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        adapter = HttpGatewayAdapter(
            "http://localhost:8000", session, max_attempts=3
        )
...
@pytest.mark.asyncio
async def test_payload_size_capping() -> None:
    """Verify response exceeds limit (1MB) triggers PAYLOAD_TOO_LARGE."""
    async with aiohttp.ClientSession() as session:
        # Set a small limit for testing
        adapter = HttpGatewayAdapter(
            "http://localhost:8000", session, max_response_size=100
        )

        with aioresponses() as m:
            # We must provide Content-Length header to trigger pre-json() check
            m.get(
                "http://localhost:8000/health/ready",
                status=200,
                body="x" * 200,
