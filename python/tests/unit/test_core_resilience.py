...
class TestStateStore:
    ...
    def test_fatal_error_reducer(self):
        store = StateStore()
        event = ErrorOccurred(source="audit", kind="AUTH", message="403 Forbidden", is_fatal=True)
        store.reduce(event)
        
        # Reducer prefixes fatal errors
        assert "403 Forbidden" in store.audit.error
        assert store.is_fatal is True
        assert "403 Forbidden" in store.global_error

@pytest.mark.asyncio
class TestCoordinator:
    async def test_max_attempts_leads_to_fatal(self):
        store = StateStore()
        gateway = MagicMock()
        audit = MagicMock()
        
        # Mock a transient failure forever for gateway health
        gateway.get_health = AsyncMock(side_effect=TuiError(kind="TRANSIENT", message="500 Internal"))
        
        coord = Coordinator(store, gateway, audit, max_handshake_attempts=2)
        coord._contracts_version_gate = "1"
        ...
        assert "Max attempts" in store.gateway.error

    async def test_auth_failure_leads_to_fatal_instantly(self):
        store = StateStore()
        gateway = MagicMock()
        audit = MagicMock()
